...
bool DeliverClaimToWaiter(Bloodborne::SummonBroker& broker, ClaimWaiters& waiters,
    ...
    const auto result =
        broker.Advertise(waiter->body, waiter->rawBody, QDateTime::currentMSecsSinceEpoch());
...
void RegisterBloodborneRoutes(QHttpServer& http, bool seamlessCoop) {
    ...
    http.route(
        ...
        [broker, waiters,
         &http](const QHttpServerRequest& request) -> QFuture<QHttpServerResponse> {
            ...
            if (!body || !Bloodborne::HasRequiredAdvertisementFields(*body)) {
                return ReadyFuture(InvalidRequest(QStringLiteral("Invalid summon advertisement")));
            }
            ...
            if (!result.pendingClaim.isEmpty()) {
                ...
                if (result.claimDeliveries > 1) {
                    ...
                    qWarning() << "Bloodborne summon: this is delivery" << result.claimDeliveries
                               << "of the same claim to user"
                               << Integer(*body, QStringLiteral("UserId"))
                               << "-- earlier deliveries were not acted on";
                }
                return ReadyFuture(RawJsonResponse(response, result.pendingHostPlacement));
            }
            if (!result.pendingHostPlacement.isEmpty()) {
                ...
                return ReadyFuture(SummonResponse(QStringLiteral("SummonDataCreateResponse"),
                                                  result.pendingHostPlacement));
            }
            ...
            if (holdMs <= 0) {
                qInfo() << "Bloodborne summon: advertised user" << userId << "session" << sessionId;
                return ReadyFuture(SummonResponse(QStringLiteral("SummonDataCreateResponse")));
            }
            ...
            qInfo() << "Bloodborne summon: advertised user" << userId << "session" << sessionId
                    << "-- holding create for up to" << holdMs << "ms";
    ...
    http.route("/summon_messenger/request", QHttpServerRequest::Method::Post,
               [broker, waiters](const QHttpServerRequest& request) -> QHttpServerResponse {
                   ...
                   case Bloodborne::SummonBroker::ClaimStatus::Claimed: {
                       ...
                       const QString key = Bloodborne::SummonBroker::KeyFor(result.targetSessionId,
                                                                            result.targetUserId);
